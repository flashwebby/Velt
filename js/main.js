...
mm.add('(min-width: 769px) and (prefers-reduced-motion: no-preference)', () => {
  ...
  gsap.from('.product-card', { y: 36, opacity: 0, duration: .7, stagger: .1, ease: 'power3.out', scrollTrigger: { trigger: '.collection-grid', start: 'top 78%', toggleActions: 'play none none reverse' } });

  // The cover remains fixed while the typographic field streams behind it.
  gsap.timeline({ scrollTrigger: { trigger: '.marquee-section', start: 'top top', end: 'bottom bottom', scrub: true, pin: true } })
  ...
  gsap.timeline({ scrollTrigger: { trigger: '.signal-section', start: 'top top', end: 'bottom bottom', scrub: true, pin: true } })
    .to(words[0], { xPercent: -105, ease: 'none' }, .12)
    .to(words[1], { xPercent: 105, ease: 'none' }, .12)
    .to('.signal-image', { width: '47vw', ease: 'none' }, .12)
    .to('.signal-secondary', { clipPath: 'polygon(0 0,100% 0,100% 100%,0 100%)', ease: 'none' }, .62);
});
